...
zend_class_entry *jalali_date_ce;

ZEND_DECLARE_MODULE_GLOBALS(jalali)
...
static PHP_MINIT_FUNCTION(jalali)
{
#if PHP_VERSION_ID < 50500
...
#ifndef ZEPHIR_RELEASE
static PHP_MSHUTDOWN_FUNCTION(jalali)
{
 ...
 */
static void php_zephir_init_globals(zend_jalali_globals *zephir_globals TSRMLS_DC)
{
...
static PHP_RINIT_FUNCTION(jalali)
{

	zend_jalali_globals *zephir_globals_ptr = ZEPHIR_VGLOBAL;
...
static PHP_RSHUTDOWN_FUNCTION(jalali)
{
...
static PHP_MINFO_FUNCTION(jalali)
{
	php_info_print_box_start(0);
	php_printf("%s", PHP_JALALI_DESCRIPTION);
	php_info_print_box_end();

	php_info_print_table_start();
	php_info_print_table_header(2, PHP_JALALI_NAME, "enabled");
	php_info_print_table_row(2, "Author", PHP_JALALI_AUTHOR);
	php_info_print_table_row(2, "Version", PHP_JALALI_VERSION);
	php_info_print_table_row(2, "Powered by Zephir", "Version " PHP_JALALI_ZEPVERSION);
	php_info_print_table_end();


}

static PHP_GINIT_FUNCTION(jalali)
{
	php_zephir_init_globals(jalali_globals TSRMLS_CC);
}

static PHP_GSHUTDOWN_FUNCTION(jalali)
{

}

zend_module_entry jalali_module_entry = {
	...
	PHP_MINIT(jalali),
#ifndef ZEPHIR_RELEASE
	PHP_MSHUTDOWN(jalali),
#else
	NULL,
#endif
	PHP_RINIT(jalali),
	PHP_RSHUTDOWN(jalali),
	PHP_MINFO(jalali),
	PHP_JALALI_VERSION,
	ZEND_MODULE_GLOBALS(jalali),
	PHP_GINIT(jalali),
	PHP_GSHUTDOWN(jalali),
	NULL,
	STANDARD_MODULE_PROPERTIES_EX
};

#ifdef COMPILE_DL_JALALI
ZEND_GET_MODULE(jalali)
#endif
